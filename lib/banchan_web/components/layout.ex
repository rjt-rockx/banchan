defmodule BanchanWeb.Components.Layout do
  ...
  alias BanchanWeb.Components.{Icon, Flash, Nav}
  ...
  def render(assigns) do
    ~F"""
    <div class="w-full h-screen drawer drawer-mobile lg:drawer-open">
          ...
          <ul tabindex="0" class="flex flex-col gap-2 p-2 menu menu-compact">
            <li>
              <LiveRedirect to={Routes.home_path(Endpoint, :index)}>
                  <Icon name="home" size="4" gap="2"><span>Home</span></Icon>
              </LiveRedirect>
            </li>
            {#if Accounts.active_user?(@current_user) && :admin in @current_user.roles}
              ...
              <li>
                <LiveRedirect to={Routes.report_index_path(Endpoint, :index)}>
                  <Icon name="flag" size="4" gap="2"><span>Reports</span></Icon>
                </LiveRedirect>
              </li>
              <li>
                <LiveRedirect to={Routes.denizen_index_path(Endpoint, :index)}>
                  <Icon name="users" size="4" gap="2"><span>Users</span></Icon>
                </LiveRedirect>
              </li>
              <li>
                <LiveRedirect to={Routes.beta_requests_path(Endpoint, :index)}>
                  <Icon name="inbox" size="4" gap="2"><span>Beta Requests</span></Icon>
                </LiveRedirect>
              </li>
              <li>
                <a href="/admin/dashboard" target="_blank" rel="noopener noreferrer">
                  <Icon name="layout-dashboard" size="4" gap="2"><span>Dashboard</span></Icon>
                </a>
              </li>
              {#if Application.fetch_env!(:banchan, :env) == :dev}
                <li>
                  <a href="/admin/sent_emails" target="_blank" rel="noopener noreferrer">
                    <Icon name="send" size="4" gap="2"><span>Sent Emails</span></Icon>
                  </a>
                </li>
                {!--
                <li>
            ...
            <li :if={Accounts.active_user?(@current_user)}>
              <LiveRedirect to={Routes.commission_path(Endpoint, :index)}>
                <Icon name="palette" size="4" gap="2"><span>My Commissions</span></Icon>
              </LiveRedirect>
            </li>
            <li>
              <LiveRedirect to={Routes.discover_index_path(Endpoint, :index)}>
                <Icon name="search" size="4" gap="2"><span>Discover</span></Icon>
              </LiveRedirect>
            </li>
            <li>
              <LiveRedirect to={Routes.discover_index_path(Endpoint, :index, "offerings")}>
                <Icon name="shopping-bag" size="4" gap="2"><span>Offerings</span></Icon>
              </LiveRedirect>
            </li>
            <li>
              <LiveRedirect to={Routes.discover_index_path(Endpoint, :index, "studios")}>
              <Icon name="store" size="4" gap="2"><span>Studios</span></Icon>
              </LiveRedirect>
            </li>
            <li class="menu-title">
              <span>Account</span>
